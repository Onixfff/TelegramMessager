...
namespace TelegramMessager
{
    ...
    public class Database
    {
        ...
        public async Task<List<Data>> GetData()
        {
            ...
            string query = $"SELECT if (time(Timestamp) < '08:05:00',date_format(date_sub(Timestamp, INTERVAL 1 DAY), \"%d %M %Y\"), date_format(Timestamp, \"%d %M %Y\")) as df, if (time(Timestamp) <= '20:05:00' and time(Timestamp)>= '08:05:00','день','ночь') as shift, data_52, count(dbid)-(select ifnull(sum(sum_er),0) as brak from spslogger.error_mas as ms where mr.data_52 = ms.recepte and(if (time(Timestamp) < '08:05:00',date_format(date_sub(Timestamp, INTERVAL 1 DAY), \"%d %M %Y\"),date_format(Timestamp, \"%d %M %Y\")))= ( if (time(ms.data_err) < '08:05:00',date_format(date_sub(ms.data_err, INTERVAL 1 DAY), \"%d %M %Y\"),date_format(ms.data_err, \"%d %M %Y\")))and(if (time(Timestamp) <= '20:05:00' and time(Timestamp)>= '08:05:00','день','ночь'))= (if (time(ms.data_err) <= '20:05:00' and time(ms.data_err)>= '08:05:00','день','ночь'))) as count_1, round(((count(dbid)-(select ifnull(sum(sum_er),0) as brak from spslogger.error_mas as ms where mr.data_52 = ms.recepte and(if (time(Timestamp) < '08:05:00',date_format(date_sub(Timestamp, INTERVAL 1 DAY), \"%d %M %Y\"),date_format(Timestamp, \"%d %M %Y\")))= ( if (time(ms.data_err) < '08:05:00',date_format(date_sub(ms.data_err, INTERVAL 1 DAY), \"%d %M %Y\"),date_format(ms.data_err, \"%d %M %Y\")))and(if (time(Timestamp) <= '20:05:00' and time(Timestamp)>= '08:05:00','день','ночь'))= (if (time(ms.data_err) <= '20:05:00' and time(ms.data_err)>= '08:05:00','день','ночь')))) * '4.32'), 2) as mas from spslogger.mixreport as mr where Timestamp >= '{thisDay.ToString("yyyy-MM-dd")} 08:05:00' and Timestamp < concat( date_add('{DateTime.Now.Year}-{DateTime.Now.Month}-{GetLastDay()}', interval 1 day), ' 08:05:00')  group by df,shift, data_52";
